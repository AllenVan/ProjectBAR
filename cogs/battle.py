...
class battleSystem(commands.Cog):
	...
	def spawn(self):
		self.client.enemy = random.choice(self.client._spawns['mobs'])
		self.enemy_HP = self.client.enemy['HP']
		...
		if self.client.job == None:
			...
			async def callback(interaction):
				...
				if self.enemy_HP <= 0:
					...
					footer_text=f"What will you do? Distance is {self.client.combat_skills.distance}. Your HP is {self.client.hp}. Enemy HP is {self.enemy_HP}."
			...
			if self.client.enemy:
				embed_message=discord.Embed(title=f"The {self.client.enemy['name']} didn't hear no bell.")
			else:
				self.spawn()
